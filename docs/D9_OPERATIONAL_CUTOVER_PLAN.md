...
| **B10 (HB-2026-148/149/150/151) — 7/7 dosya GERÇEKTEN onarıldı (HB-2026-151, 2026-08-05); 4 target-only dosya AYRI, çözülmemiş** | Kullanıcının açık onayıyla başlatılan gerçek Apply denemesinde, taze Adım 0 sessizlik kısmı TEMİZ geçti ama nihai tam kaynak==hedef SHA-256 karşılaştırması UYUŞMADI. Salt-okunur forensics ile izole edildi: 3 vaka klasöründe **7 repair adayı + 4 blocker + 2 kapsam dışı** (13 toplam, HB-2026-149'da ham JSON'dan kesinleşti). `repair-post-sync-stale-target-files.ps1` (HB-2026-130) o an yalnız kendi eski şemasını kabul ediyordu. | **Araç GENELLEŞTİRİLDİ (HB-2026-150):** `Get-CandidateEntriesDiffForensics` adapter'ı eklendi (eski 56AAG629 yolu ayrı fonksiyona taşındı, DEĞİŞMEDİ) — yalnız `content_mismatch`+doğru yön (`source_current_target_superseded`)+pCloud `CurrentRow`(kaynakla)+`Revisions`de AYRI superseded kayıt(hedefle, farklı hash)+`TaskReferenceCount=0` kanıtlanan dosyalar aday olur; `extra` HER ZAMAN blocker (asla dokunulmaz), `metadata_only` kapsam dışı, eksik/tanınmayan kanıt fail-closed red — üçü ayrı sayılarla raporlanır. Yol boyunca GERÇEK bir hata bulunup düzeltildi: paylaşılan taze-yeniden-doğrulama zinciri yalnız `revisions[0]`e bakıyordu ("en eski revizyon index 0'da" varsayımı) — gerçek B10 verisinde 3 dosyada iki revizyon AYNI ctime'a sahip ve pCloud CURRENT'ı index 0'da döndürüyor, bu da 3 gerçek adayı yanlışlıkla reddediyordu; kontrol artık index'e değil "CURRENT'tan farklı, hedefle eşleşen HERHANGİ bir revizyon var mı"ya bakıyor (eski davranışın kesin genellemesi). 11 test (6 eski değişmeden geçiyor + 5 yeni, sıra-bağımsızlık regresyonu dahil) + statik denetim, hepsi geçti. **Gerçek makinede kanıtlandı (yalnız salt-okunur önizleme, `-Apply` VERİLMEDEN):** gerçek B10 raporuna karşı `WouldApplyCount=7, BlockedCount=0, ClassificationBlockedCount=4, OutOfScopeCount=2` — HB-2026-149'un manuel analiziyle birebir aynı. Sıfır dosya yazıldı/silindi/değiştirildi. **Gerçek onarım Apply'ı bu pakette ÇALIŞTIRILMADI** — ayrı, açık kullanıcı onayı gerektiriyor (AGENTS.md §7); 4 blocker için de ayrı bir karar/araştırma gerekiyor. |
