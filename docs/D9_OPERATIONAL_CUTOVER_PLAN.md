...
| **B10 (HB-2026-148/149'da bulundu) — GERÇEK Apply denemesi #1'i Adım 0'da durdurdu** | Kullanıcının açık onayıyla başlatılan gerçek Apply denemesinde, taze Adım 0 (`test-pcloud-post-sync-rebaseline-gate.ps1`) çalıştırıldı: sessizlik kısmı TEMİZ geçti (`ObservedQuietSeconds=689`, `WindowResetCount=0`), ama nihai tam kaynak==hedef SHA-256 karşılaştırması UYUŞMADI. Salt-okunur `run-pcloud-post-sync-diff-forensics.ps1` ile izole edildi: 6996/7000 dosyadan 6987 özdeş; ham JSON'un 13 kaydı (Administrators-only dosyadan bizzat okunarak, metin özetine güvenilmeden doğrulandı) **7 repair adayı + 4 blocker + 2 kapsam dışı**a ayrıştı (HB-2026-149) — 3 vaka klasöründe. | **Çözülmedi, gerçek bir ürün/operasyon kararı gerektirir** (bu pakette hiçbir onarım/Apply YAPILMADI): **7 repair adayı** — pCloud'un TAZE `currentRow`u (yeniden hesaplanmış SHA-256/boyutla) kaynakla, `revisions` geçmişindeki AYRI bir kayıt hedefle eşleşiyor, `taskReferenceCount=0` — 6'sı hedefte sıfır bayt, 1'i (iki tarafta da dolu) yalnız pCloud'un gerçek iki revizyonu (63328B→280892B) sayesinde aday (revision kanıtı YOK ise aday sayılmayacaktı). **4 blocker** (aynı vaka klasöründeki 4 belge) — kaynakta karşılığı yok, taze pCloud sorgusunda da `found:false`; silinmedi/değiştirilmedi. **2 kapsam dışı** — içerik SHA-256'sı aynı, yalnız mtime farklı. **Yeni bulgu:** `repair-post-sync-stale-target-files.ps1` (HB-2026-130) yalnız kendi eski `hasarbotu-56aag629-hasar-version-forensics/1.0.0` şemasını kabul ediyor — bu olayın genel `pcloud-post-sync-diff-forensics/1.0.0` raporunu `FORENSICS_REPORT_SCHEMA_INVALID` ile reddediyor (kod değiştirilmeden kullanılamaz). Tam dosya listesi (kesin yollar/vaka numaraları dahil) yalnız Administrators-only+hash'li rapordadır; kullanıcıya sohbet içinde tam detayla raporlandı — repo/commit'e (HB-2026-123 ilkesiyle) ALINMADI. **B10 çözülmeden Adım 0 tekrar PASS veremez, D9'un geri kalanı (Adım 1-6) başlatılamaz.** |
