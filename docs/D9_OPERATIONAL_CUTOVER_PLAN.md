...
| **B10 (HB-2026-148…157) — 17/17 dosya GERÇEKTEN onarıldı; kalan 5 extra dosya kanıtlandı, cleanup aracı HAZIR (yalnız önizleme, 2026-08-05)** | Post-sync forensics'te gerçek, sürekli tekrarlanan bir desen: bazı dosyalar hedefte "stale" (kaynak güncellendi/değişti, hedef eski/boş kaldı — `repair-post-sync-stale-target-files.ps1` konusu), bazıları "extra" (hedefte var ama kaynakta hiç karşılığı yok — ayrı bir araç gerektirir). | **Repair yolu (HB-2026-149/150/151/155/156):** Genel `pcloud-post-sync-diff-forensics/1.0.0` şeması için adapter eklendi (eski 56AAG629 yolu değişmedi), yol boyunca bulunan gerçek bir `revisions[0]`-sıra-bağımlılığı hatası düzeltildi. **Gerçek `-Apply` iki kez çalıştırıldı: toplam 17/17 dosya başarıyla onarıldı** (7 + 17, HB-2026-151/156), sıfır sürüklenme, bağımsız SHA-256 doğrulamasıyla teyitli, orijinal içerik sync kökü dışında Administrators-only+hash'li yedeklerde korunuyor. **Cleanup yolu (HB-2026-157):** Kalan 5 `extra` dosya salt-okunur köken doğrulamasıyla kesin sınıflandırıldı — 2'si `rename_artifact` (içerik+oluşturma zamanı, aynı vakada hâlâ kaynakta var olan yeniden adlandırılmış dosyayla birebir eşleşiyor), 3'ü `stale_duplicate` (içerik, aynı vakanın kendi `OLAY YERİ` alt klasöründe hâlâ doğru senkronize duran kopyayla birebir eşleşiyor) — hepsi pCloud canlı ağacında `found:false`, kilitli değil, kuyruk referansı sıfır. Yeni `cleanup-post-sync-target-only-files.ps1` (+ 6 testi) yazıldı — yalnız `extra` sınıflı kayıtları işler, taze yeniden doğrulama + sync kökü dışında admin-only+hash'li yedek + `[System.IO.File]::Delete` + bağımsız doğrulama. **Gerçek makinede kanıtlandı (yalnız salt-okunur önizleme, `-Apply` VERİLMEDEN):** `WouldDeleteCount=5, BlockedCount=0`. **Gerçek silme Apply'ı bu pakette ÇALIŞTIRILMADI** — ayrı, açık kullanıcı onayı gerektiriyor (AGENTS.md §7). Onay sonrası kalan tek fark türü 5 benign `metadata_only` (içerik aynı, yalnız mtime) olacak. |
