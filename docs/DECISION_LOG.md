...
**Guncelleme (2026-08-09/10, ayni oturum): kullanici tek onayi verdi, gercek `-Apply` GERCEKTEN calistirildi.** `harden-api-service-directory-acl.ps1 -Apply` -> `Status:"applied"`, `BroadIdentitiesRemaining:[]`. Bagimsiz, ayri salt-okunur `Get-Acl` sorgulariyla dogrulandi: root'ta TAM 2 ACE (`NT AUTHORITY\SYSTEM: ReadAndExecute+Synchronize`, `BUILTIN\Administrators: FullControl`, ikisi de `IsInherited=False`); `logs\`de TAM 2 ACE (`SYSTEM: Modify+Synchronize`, `Administrators: FullControl`); eski `Authenticated Users`/`Users` genis grantlari TAMAMEN yok. `hasarbotu-api` servisi dogrulama sonrasi hala `Running`/`Automatic` -- Apply servise dokunmadi (yalniz NTFS ACL degisti). Kanit raporu Administrators-only ACL ile `C:\ProgramData\HasarBotu\migration-preflight\harden-api-directory-acl-snapshot-2026-08-09T214655.329Z.json`e yazildi (SHA-256: `64d5e3af...`). **Faz 1'in iki kayitli artik riskinden ikincisi de (API dizini ACL) artik tamamen COZULDU** -- File Agent ile ayni en-az-yetki disiplinine ulasildi.

## 2026-08-10 - HB-2026-195: Windows NSIS installer paketleme altyapisi SIFIRDAN kuruldu -- kullanicinin acik onayiyla, gercek install/launch/uninstall/reinstall dogrulamasiyla

Karar:

1. Depoda HICBIR installer/EXE paketleme altyapisi yoktu (electron-builder/forge kurulu degildi, hicbir roadmap/mimari belgesinde installer/imzalama/guncelleme karari yoktu -- `docs/DEPLOYMENT_AND_OPERATIONS_PLAN.md`in kendi "Acik karar kapilari" tablosunda bile yer almiyordu; `apps/desktop/src/main/main.ts`in kendi yorumu bile "code signing, installer ve otomatik guncelleme YOKTUR (Paket 21/22 dagitim kararlari)" diyordu). Bu, belgelerde cevabi olmayan gercek bir urun karariydi -- kullaniciya 3 soru soruldu, ucu de onaylandi: (a) electron-builder+NSIS ile sifirdan kur, (b) imzasiz devam et (SmartScreen "Bilinmeyen Yayimci" kabul edilebilir), (c) surum 0.1.0 (`0.1.0-ui-baseline` korunan tag'inden BAGIMSIZ, yalniz `apps/desktop` paketinin kendi surumu).
2. `apps/desktop/package.json`e `electron-builder@26.15.3` (exact-pin, `electron` ile ayni disiplin) + `build` alani eklendi: `appId: com.hasarbotu.desktop`, NSIS (`oneClick:false`, `perMachine:false` -- yonetici yukseltmesi gerektirmez, kullanicinin kendi hesabina kurulur, `allowToChangeInstallationDirectory:true`), `extraResources` kok `dist/`i (Vite UI build ciktisi) pakete `resources/dist/` olarak kopyalar.
3. `apps/desktop/src/main/main.ts`teki `defaultAssetRoot()` paketlenmis moda uyarlandi: `app.isPackaged` ise `process.resourcesPath/dist`, degilse ONCEKI (degismemis) gelistirme-modu goreli yolu. Bu, kodun kendi "paketlenmis dagitimda varsayim yapilmaz" yorumunun ongordugu, standart Electron idiyomuyla kapatilan bosluktu -- `HASARBOTU_ASSET_ROOT` ortam degiskeni HALA her iki modda da oncelik tasir, degismedi.
4. Imza YOK (dogrulandi: `Get-AuthenticodeSignature` -> `NotSigned`, hem installer hem ana exe icin, kullanicinin acik karariyla tutarli).

Gerekce:

Kullanicinin acikca istedigi "final Windows production paketlemesi": gercek bir installer/EXE uretilsin, API+File Agent+desktop uyumu dogrulansin, mevcut production bozulmasin, install/upgrade/uninstall senaryolari test edilsin, hash uretilsin, smoke test yapilsin.

Etkisi (gercek, calistirilarak dogrulandi -- rapor edilmedi, olculdu):

- **Bagimlilik grafi dogrulamasi:** `npx asar list` ile app.asar icinde `@hasarbotu/contracts` (508 dosya), `@hasarbotu/domain` (160), `@hasarbotu/desktop-bridge` (16), `zod` (642) GERCEK dosya olarak (kirik symlink degil) dogrulandi; npm workspace monorepo + electron-builder'in bilinen zorluk noktasi boylece kanitlandi, varsayilmadi.
- **Tam ucdan uca smoke (gercek, `hasarbotu_test`e bagli, TAMAMEN ayri, gecici bir yerel API sureciyle -- port 3101, gercek `hasarbotu-api`/`hasarbotu-file-agent` Windows Service'lerine HIC dokunulmadi):** installer sessiz kuruldu (`/S`) -> `%LOCALAPPDATA%\Programs\HasarBotu\HasarBotu.exe` gercekten baslatildi -> kopru portu (`Get-NetTCPConnection`) bulundu -> `http://127.0.0.1:<port>/` GERCEKTEN `<title>HasarBotu V2</title>` iceren dogru `index.html`i dondurdu (paketlenmis asset-root duzeltmesinin dogrudan kaniti) -> `/api/v1/cases` GERCEKTEN 401 dondurdu (proxy'nin gercek API'ye ulastiginin kaniti, sahte degil).
- **Kurulum/kaldirma dongusu (4 kez, gercek):** ilk kurulum -> Start Menu + Desktop kisayolu + registry uninstall kaydi (`DisplayVersion 0.1.0`) dogrulandi -> tam kaldirma -> TUM izler (dizin, kisayollar, registry) sifirlandigi bagimsizca dogrulandi -> yeniden kurulum UZERINE kurulum (upgrade-senaryosu yerine gecen tek gercekci test, cunku bu ILK surum, ONCEKI bir surum yok) hatasiz gecti -> son kaldirma temiz.
- **BEKLENMEYEN, ONEMLI bulgu (test sirasinda, salt-okunur kesif):** bu makinede ZATEN calisan, farkli bir uygulama var -- `HasarBotu - Baran Ekspertiz 0.6.10` (`C:\Program Files\HasarBotu - Baran Ekspertiz\`, per-machine, Publisher `Baran Global Ekspertiz`, 4 surec 8.08.2026'dan beri calisiyor). Bu neredeyse kesinlikle AGENTS.md'nin "V1 aktarimi" basligindaki GERCEK, halen kullanimda olan V1 sistemi -- bu repodaki HicBir sey degil. Isim (`HasarBotu` vs `HasarBotu - Baran Ekspertiz`), kurulum konumu (`%LOCALAPPDATA%` vs `Program Files`), registry `DisplayName` ve surec HICBIRI CAKISMADI; V1 sureçleri test boyunca kesintisiz calismaya devam etti (bagimsizca dogrulandi). **Ileri urun riski (bu paketin kapsami disinda, disclosed):** V2 bu makineye GERCEKTEN dagitilirsa iki "HasarBotu" uygulamasi ayni ofis calisaninin Start Menu'sunde gorunecek -- isim ayrimi (orn. "HasarBotu V2" veya "HasarBotu - Baran Ekspertiz V2") ayri bir urun karari gerektirir.
- Typecheck/lint/test/build/check:deploy TAM yeniden calistirildi paketleme degisikliklerinden SONRA: 2.260 test, 0 basarisiz (paketleme oncesiyle BIREBIR ayni sayi -- regresyon yok).
- `.gitignore`e `release/` eklendi (electron-builder ciktisi asla commit edilmeyecek).
...
## 2026-08-11 - HB-2026-198: SYSTEM_PRODUCT_INVENTORY_REPORT denetiminde bulunan 2 gercek kusur duzeltildi -- labor-allocation-ai Gemini wiring + kapali-dosya PATCH guard, tam gercek Postgres+HTTP kapi dogrulamasiyla

Karar:

1. Bir onceki paketin (salt-okunur, 11 paralel ajanli sistem envanteri denetimi, `docs/SYSTEM_PRODUCT_INVENTORY_REPORT.md`) bulup rapora isledigi iki gercek kod kusuru, kullanicinin acik talimatiyla paketlemeden once duzeltildi.
2. **Kusur 1 -- labor-allocation-ai Gemini adaptoru production'a bagli degildi:** `services/api/src/server.ts`in gercek `startServer()` giris noktasi, tam yazilmis/test edilmis `createConfiguredLaborAllocationProviderRegistry()` fonksiyonunu tanimliyordu ama `buildApp()` cagrisina hic gecirmiyordu; sonuc `buildApp()`un kendi (yalniz test amacli) `createDeterministicLaborAllocationProviderRegistry()` varsayilaninin production'da sessizce devreye girmesiydi -- kullanici, her satiri sabit `controlRequired:true`+%55 guvenle isaretleyen bir test-fixture cikisini gercek AI cevabi sanabilirdi. **Duzeltme:** `server.ts` artik `laborAllocationProviders: createConfiguredLaborAllocationProviderRegistry(config)` + (yapilandirilmissa) `laborAllocationProviderId: GEMINI_POLICY_PROVIDER_ID` ile registry'yi gercekten kuruyor; `index.ts` fonksiyonu disa aktariyor (kardesleriyle -- `createConfiguredPolicyAiProviderRegistry`/`createConfiguredEmailAiProviderRegistry` -- ayni listede). `buildApp()`un kendi test-amacli varsayilanina VE mevcut ~20 test dosyasinin kendi registry'sini acikca gecirme sozlesmesine KASITLI OLARAK dokunulmadi -- duzeltme yalniz gercek `startServer()` giris noktasini hedefliyor.
3. **Kusur 2 -- kapali dosyada temel `cases` alan guncellemesi reddedilmiyordu:** `cases/write-store.ts`teki `updateCase()` (`PATCH /api/v1/cases/:caseId`) `lifecycle_status`u hic kontrol etmiyordu -- sorumlu/servis/takip-tarihi gibi alanlar kapali bir dosyada da degistirilebiliyordu, diger 10+ modulun (case-operations/labor/pert/email-drafts/kasco-mandatory-check/traffic-value-loss/...) uyguladigi fail-closed sozlesmenin disinda kaliyordu. **Duzeltme:** ayni `FOR UPDATE` satiri artik `lifecycle_status`u da okuyor, `open` degilse `{kind:'case_closed'}` ile ROLLBACK edip 409 `conflict`/"Closed cases cannot be updated." donduruyor -- kardes modullerle (`case-operations/store.ts`teki `lockWritableCase` deseni) birebir ayni sozlesme, ayni HTTP kodu/mesaj uslubu.
4. **Kapsam disi birakilan, ayri bir bosluk (bilinerek dokunulmadi):** `labor-ai` (Paket 44, Iscilik'in ayri/daha basit ilk-oneri modulu) icin hicbir `GEMINI_LABOR_AI_*` config yuzeyi hic yazilmamis -- bu "unutulmus kablo" degil, muhtemelen henuz production'a hazirlanmamis ayri bir modul; ayri bir gelistirme karari gerektirir.
5. **Yedekleme konusu -- YENI OZELLIK EKLENMEDI:** kullanici acikca otomatik/zamanlanmis yedekleme eklenmemesini istedi; mevcut urun karari (aktif depolama yerel NTFS, pCloud senkron/arsiv MANUEL operasyonel surec -- D5-D9 kararlari) korunuyor. Rapor bu konuyu onceki surumunde yanlislikla "en somut production riski" diye nitelendirmisti -- `DEFERRED / INTENTIONAL MANUAL POLICY` olarak duzeltildi.
6. Rapor (`docs/SYSTEM_PRODUCT_INVENTORY_REPORT.md`) yalniz gercekten degisen bolumlerde (SS4, 6, 16.3, 24, 28, 29, 30, 31, 33, kapanis listeleri) guncellendi; geri kalani oldugu gibi korundu.

Gerekce:

Kullanicinin paketleme oncesi dogrudan talimati: rapor onaylandi, iki bulgu "gercek kusur" olarak nitelendirilip kapatilmasi istendi; yedekleme konusunda ise YENI ozellik eklenmemesi, yalniz raporun yanlis siniflandirmasinin duzeltilmesi istendi.

Etkisi:

- Degisen dosyalar: `services/api/src/server.ts`, `services/api/src/index.ts`, `services/api/src/cases/write-store.ts`, `services/api/src/cases/write-routes.ts`, `services/api/test/labor-allocation-provider-optin.test.ts` (+3 yeni test), `services/api/test/cases-write.test.ts` (+1 yeni test), `docs/SYSTEM_PRODUCT_INVENTORY_REPORT.md`.
- **Gercek dogrulama (tam, bu oturumda gercekten calistirildi):** `npm run typecheck` (tum workspace'ler: domain/contracts/database/desktop-bridge/kok-UI/api/file-agent/desktop) 0 hata; `npm run lint` 0 hata (13 onceden var olan, hic degismemis dosyalardaki uyari); `npm run build` (UI + 7 paket) basarili, bundle butcesi gecti (baslangic 426.198 bayt, 10 lazy modul ayri); `npm run check:deploy` gecti; `npm audit` yalniz onceden bilinen, yapisal olarak erisilemez tek `pdfjs-dist` bulgusu (degismedi). **Tam test suite (gercek `hasarbotu_test` PostgreSQL'e karsi, tum 8 workspace zincir halinde): src 386/392 (6 ortam-kosullu skip), domain 768, contracts 324, database 74, desktop-bridge 10, api 520 (78 dosya), file-agent 110, desktop 75 -- TOPLAM 2.267 PASS, 6 skip, 0 basarisiz.** (Onceki taban 2.260'ti -- HB-196/197 commit'lerinden 3 test artisi + bu paketin 4 yeni testi.)
- Yeni testler gercek AI/ag cagrisi yapmadan (bos registry veya spy adapter) hem birim hem gercek Postgres+HTTP seviyesinde dogrulaniyor; labor-allocation-ai regresyon testi arka-plan (fire-and-forget) analiz surecini terminal duruma ulasana kadar yoklayarak dogru sekilde bekliyor.
- Production secret okunmadi/yazilmadi (yalniz yerel `%USERPROFILE%\.hasarbotu\hasarbotu_test.pass`, DATABASE_OPERATIONS.md'de belgelenen izole test rolu); gercek AI saglayicisina hicbir cagri yapilmadi; gercek production DB/servis/dosya sistemine dokunulmadi.
- **Kalan gercek risk (bu paketin kapsaminda degil, guncel):** `labor-ai`nin ayri Gemini wiring bosluğu (ayri karar gerektirir), bootstrap-sonrasi kullanici-olusturma API/UI eksikligi, `hasarbotu-api` servis hesabinin LocalSystem olmasi -- PROJECT_STATUS/rapor SS30'da guncel.

Kaynak: 2026-08-11 tarihli kullanici talimati ("SYSTEM_PRODUCT_INVENTORY_REPORT incelemesi kabul. Paketlemeden once raporda bulunan iki gercek kusuru kapat...").

## 2026-08-11 - HB-2026-199: KRITIK production blocker duzeltildi -- V1 plaka klasoru sidecar verisi (notlar/gorevler/sorumlu/takip tarihi) V2'ye hic tasinmiyordu; kalici, idempotent, test edilmis import/backfill sistemi yazildi, gercek prod'da salt-okunur preview calistirildi, Apply UYGULANMADI

Karar:

1. Kullanicinin paketleme SONRASI bildirdigi kritik blocker uzerine paketleme/kullanim durduruldu; kok neden tahmin edilmeden kod/tarihce uzerinden dogrulandi: HB-2026-197'nin "V1 aktarimi" `git show` ile incelendi -- silinmis bir tek-seferlik script yalniz dosya TURUNU (trafik/kasko) okuyup PATCH ediyordu, `_HASARBOTU/takip.json` icindeki notlar/gorevler/sorumlu-eksper/takip tarihi/ihbar no/kapanis durumu dahil HER SEY sessizce atiliyordu ve bu hicbir yerde (rapor, UI, log) kullaniciya surulmuyordu.
2. V1 sidecar semasi ("schemaVersion":1) gercek production dosyalarindan (tahmin edilmeden) tersine cikarildi; `HASARBOTU_TAKIP_OZETI.txt`nin kendi son satiri takip.json'i tek kaynak, kendisini salt-okunur ozet olarak tanimliyor -- JSON/TXT celiskisinde JSON'un kazanmasi bu YUZDEN veriden dogrulanan bir kural, tahmin degil.
3. Kalici kod yazildi (4 atomik commit, HB-2026-199 kapsaminda -- **onceki 4 commit mesaji yanlislikla HB-2026-198'e referans veriyor; o ID zaten Paket'in bir onceki (labor-allocation/kapali-dosya) girisine ait, duzeltme sonraki commit'lerde yapilmayacak (mevcut commit amend edilmiyor), gercek takip numarasi budur -- HB-2026-199**):
   - `packages/database/migrations/0046_v1_import_provenance.js`: `v1_import_records` provenance/idempotency tablosu, append-only (mevcut `append_only_guard` yeniden kullanildi), benzersizlik kisiti `(organization_id, source_relative_path, item_type, source_item_id)`.
   - `packages/domain/src/v1-import.ts`: saf karar cekirdegi -- `parseV1PlateFolderName` (mevcut `parsePlateNumber` klasor-adi siniflandirmasi icin fazla toleransli oldugu icin katı sekil kontroluyle sikilastirildi), `mapV1ClaimType`, `deriveV1ClosedState`, `decideV1FieldBackfill` (V2 alani BOSSA safe_backfill, DOLUYSA conflict -- asla kor ezme).
   - `services/api/src/v1-import/{schema,store,index}.ts`: DB-farkinda orkestrasyon -- `discoverV1Folders`/`readV1Sidecar` tamamen salt-okunur (readFile/readdir/stat), `planV1Import` (0 yazma) ve `applyV1Import` (her klasor kendi transaction'inda, once TAZE TOCTOU kontrolu, sonra yazma).
   - `deploy/windows-service/run-v1-import.mjs`: CLI. Varsayilan PREVIEW (0 DB yazmasi). `--apply` yalniz gercek TTY'de VE tam "UYGULA" yazilan onayla calisir; onaydan SONRA plan TAZE yeniden hesaplanir (onay yazma suresindeki TOCTOU'ya karsi).
4. Gelistirme sirasinda kendi test paketimle (tahmin degil) 2 gercek kusur bulup duzelttim:
   - **follow_up_date hic yazilmiyordu:** case-olusturma INSERT'i `entry.fieldBackfills.find(f=>f.field==='followUpDate')`e bakiyordu ama `fieldBackfills` yalniz `backfill_existing` yolunda dolduruluyor, `create_case`da hep bos kaliyordu -- yeni case'ler V1 takip tarihini ASLA almiyordu. Duzeltme: plan entry'sine ayri `followUpDateCandidate` alani eklendi, INSERT dogrudan onu okuyor.
   - **Ayni plakanin " - N" sonekli AYRI V1 klasorleri (AGENTS.md'de belgeli desen) yanlislikla tek case'e dusuyordu:** apply-oncesi TOCTOU kontrolu `plate_normalized` ile "bu plakada herhangi bir case var mi" diye soruyordu -- ayni apply kosusunda ONCE islenen kardes klasorun yarattigi case, SONRAKI kardes icin sahte bir "cakisma" gibi gorunup onu sessizce atliyordu (plaka bu sistemde benzersiz kimlik DEGIL -- "Her vaka ayri caseId alir"). Duzeltme: kontrol artik `v1_import_records.source_relative_path` ile TAM BU KLASORUN daha once case'e donusup donusmedigine bakiyor.
5. Migration 0046'nin repo'ya EKLENMESI (zincirin sonuna, yeni HEAD) 2 onceden var olan test dosyasinin migration-sayisi varsayimlarini kaydirdi -- ikisi de duzeltildi, mantik degistirilmedi: `packages/database/test/integration.test.ts`nin zaten bu amacla var olan `HIDDEN_MIGRATION_NAMES` genisleme noktasina '0046_v1_import_provenance' eklendi (+bastan-migrate testinin tam tablo listesine `v1_import_records`); `services/api/test/traffic-value-loss-hardening.test.ts`nin 0043'un rollback guard'ina ulasmak icin sabit kodladigi `count:3` -> `count:4` (+reapply sonrasi beklenen isim listesi).
6. **Gercek production DB durum tespiti (salt-okunur), beklenmeyen bulgu:** preview'i gercek prod DB'ye karsi calistirmadan once `pgmigrations` sorgulandi -- prod HEAD `0044`de kalmisti, **0045_kasco_mandatory_checks de (onceden yazilmis/test edilmis, ayri bir ozellik) hic prod'a uygulanmamisti**, muhtemelen Faz 3 paketlemesi yalniz masaustu installer'i kapsadigi icin. Migration'lar sira disi calisamadigindan 0046'ya ulasmak 0045'i de zorunlu kiliyordu. Kullaniciya IKI AYRI soru soruldu (once "0046'yi uygula mi", sonra "0045 de bekliyor, birlikte mi") -- ikisi de acikca onaylandi ("Additive migration'i simdi uygula (Onerilen)"). Her ikisi de SADECE yeni bos tablo (CREATE TABLE), mevcut hicbir tabloya/veriye ALTER/UPDATE/DROP yok; uygulama sonrasi `cases` satir sayisi (164) degismedigi dogrulandi.
7. **Gercek, salt-okunur preview** (`run-v1-import.mjs`, `--apply` YOK) gercek `C:\HasarBotuStorage\BARAN GLOBAL EKSPERTİZ` ve gercek prod DB'ye karsi calistirildi -- bkz. asagida tam sayilar. Bir sonuc rastgele bir yeni-olusturulan gercek klasorle (`34YYY36`, bugun 2026-08-11 saat 07:46'da personel tarafindan olusturulmus, todos/notes bos) elle capraz kontrol edildi -- plan ciktisi (`ClaimType: traffic, NewNotes:0, NewTasks:0`) dosyanin ham JSON'uyla birebir eslesti.
8. **Apply calistirilmadi.** Kullanicinin acik, tekrarlanan talimati geregi gercek veri yazma adimi (case/not/gorev olusturma) bu oturumda TETIKLENMEDI -- yalniz bu iki DDL migration (adim 6) ve salt-okunur preview (adim 7) calisti.

Gerekce:

Kullanicinin acik talimati: paketleme/kullanim durdurulup kok neden tahmin edilmeden arastirilacak, kalici/idempotent/onizlemeli bir cozum yazilacak, gercek prod DB Apply noktasinda durup TEK onay istenecekti. V1 kaynagi hicbir zaman yazilmayacak, ikinci calistirma duplicate uretmeyecek, mevcut V2 degeri kor ezilmeyecek sekilde tasarlanmasi ayrica zorunlu kilindi.

Etkisi:

- **Yeni/degisen dosyalar (4 commit):** `packages/database/migrations/0046_v1_import_provenance.js`; `packages/domain/src/v1-import.ts` + `packages/domain/src/index.ts` + `packages/domain/test/v1-import.test.ts`; `services/api/src/v1-import/{schema,store,index}.ts` + `services/api/src/index.ts` + `services/api/package.json` (+zod, contracts ile ayni surum) + `package-lock.json` + `services/api/test/v1-import.test.ts` + `deploy/windows-service/run-v1-import.mjs`; `packages/database/test/integration.test.ts` + `services/api/test/traffic-value-loss-hardening.test.ts` (migration-sayisi duzeltmeleri).
- **Gercek dogrulama (bu oturumda calistirildi):** `npm run typecheck` (tum workspace'ler) 0 hata; `npm run lint` 0 hata (13 onceden var olan, degismemis dosyalardaki uyari, `pdfjs-dist` audit bulgusu onceden bilinen/kapsam disi). **Tam test suite (gercek `hasarbotu_test` PostgreSQL'e karsi): domain 784, contracts 324, database 74, desktop-bridge 10, api 529 (79 dosya, v1-import.test.ts'nin 9'u dahil), file-agent 110, desktop 75 -- TOPLAM 1.906 PASS, 0 basarisiz.**
- **V1 VERI KESIF SAYILARI (gercek prod storage + gercek prod DB, salt-okunur, 2026-08-11T12:43:11.755Z):** toplam klasor 202; JSON'lu 140, TXT'li 140, ikisi de yok (missing_sidecar) 61, klasor-adi plaka olarak ayristirilamayan 1 (`PLAKA` -- sablon/placeholder klasor), bozuk/desteklenmeyen JSON 0.
- **PLAN DAGILIMI:** yeni case olusturulacak 2; mevcut V2 case'e backfill 87 (bunlarin HICBIRINDE alan-seviyesi conflict yok -- hepsi bos alan, safe_backfill); claimType bilinmiyor/bos 46; plaka belirsiz (V2de ayni plakada 2 aday, hangisi bu klasor tahmin edilmedi) 5; ice aktarilacak yeni not 463; ice aktarilacak yeni (tamamlanmamis) gorev 13; kapali V2 case oldugu icin atlanan 0.
- **MEVCUT V2 IMPORT DURUMU:** V2de zaten 164 case var (HB-2026-197'nin eski script'inden, yalniz case_type/plate ile). Bunlarin 87'sinde V1'in notlar/gorevler/sorumlu/takip-tarihi hala EKSIK (bu paket calisir hale getirmeden). 0 case `up_to_date` (yeni sistemden henuz hic Apply calismadigi icin beklenen).
- Production secret hicbir yerde loglanmadi/yazilmadi (yalniz `%USERPROFILE%\.hasarbotu\hasarbotu_app.pass`/`hasarbotu_test.pass`, ortam degiskenine okunup dogrudan kullanilan, asla print edilmeyen).
