...
| ADR-Q08 | Windows servis yönetimi ne? | WinSW; NSSM; Task Scheduler; container service | WinSW tanımlı servis; NSSM kolay; Task Scheduler yerleşik | Wrapper bakımı; Task Scheduler sağlık semantiği zayıf | **KİLİTLENDİ (HB-2026-108, 2026-07-29): WinSW.** NSSM resmi kararlı sürümü 2014'te dondu; Task Scheduler SCM sağlık/bağımlılık semantiğinden yoksun. Config/runbook: `deploy/windows-service/`, `docs/RUNBOOK_FAZ_A_WINDOWS_SERVICE_DEPLOYMENT.md`. | Otomatik başlama/restart ve ücretsiz kullanım | Aynı pakette ölçülen ayrı kısıt: mevcut `P:\` pCloud sanal sürücüsü hiçbir Windows servisinden görünmez (sarmalayıcı seçimiyle çözülmez) — çözüm pCloud senkronize klasör geçişidir |
