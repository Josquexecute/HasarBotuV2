...
- **Gerçekleşen sonuç (tamamlandı; kullanıcı "Paket 13 — belge/fotoğraf metadata" olarak yönlendirdi, 2026-07-13):** Bu paketin BELGE/SÜRÜM/FOTOĞRAF META VERİSİ tamamlandı. Migration 0007 `documents` (optimistic `version`) + `document_versions` (immutable + `ready` CHECK + previous-version + hash/size/status/verified_at) + `photos`; `metadata_append_guard` (DELETE + kayıtlı-gerçek değişimi reddi, yalnız doğrulama alanları güncellenebilir). Domain `file-metadata.ts` (dosya adı güvenliği, uzantı↔MIME allow-list, SHA-256). Contracts `v1/documents` (golden 21). API `POST /cases/:caseId/documents` (yeni belge/sürüm, optimistic lock, zorunlu Idempotency-Key, MIME/uzantı+kategori+tehlikeli-ad doğrulaması, duplicate tespiti/birleştirmesiz) + `POST .../photos` + salt-okunur `GET` liste/detay. Kayıt daima `pending`; `ready` yalnız güvenilir doğrulama sonrası (DB CHECK); merkezi audit; mutlak yol sızmaz. File Agent doğrulama akışı belgelendi (FILE_STORAGE_AND_AGENT_PLAN §2.2). +9 domain + +12 API testi + canlı smoke (HB-2026-019). Böylece "Paket 11 — Dosya meta verisi" TAM tamamlandı. Gerçek yükleme/içerik/OCR/thumbnail/AI/File Agent hâlâ kapsam dışı.
