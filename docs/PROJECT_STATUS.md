...
- **API dizini ACL:** `harden-api-service-directory-acl.ps1` için taze salt-okunur preview çalıştırıldı — canlı dizinde drift yok (Faz 1 ile birebir aynı plan). Gerçek `-Apply` kullanıcının tek açık onayını bekliyor; bu turda çalıştırılmadı.
- Commit: HB-2026-192 (database test shim), HB-2026-193 (npm audit fix). Gerçek production verisi/servisi mutate edilmedi.
