...
- **Parola:** artık operatörden alınmıyor — betik `-Apply` sırasında kriptografik RNG ile bir kez üretir, yalnız `SecureString` olarak bellekte tutar, hesaba ve SCM'ye (`sc.exe config ... password=`, tek seferlik komut satırı argümanı) aktarır; **hiçbir dosyaya/log'a/repository'ye yazılmaz**. İşlem bittikten sonra parolayı bilen hiçbir kayıt kalmaz (kasıtlı — rotasyon = betiği tekrar çalıştırmak).
