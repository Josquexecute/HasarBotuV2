...
- **Açık kalan:** ofis dağıtım makinesinde aynı probun çalıştırılıp sonucun bağımsız doğrulanması ve D5 kararının (senkron klasör geçişi gerekli mi) buna göre yeniden değerlendirilmesi — kullanıcı kararı.
...
- **Açık kalan:** ofis makinesinde aynı derin probun çalıştırılıp ACL/sürücü mimarisinin doğrulanması (farklı pCloud sürümü farklı sonuç verebilir); NTFS senkron klasöre geçişin kendisi (uygulama, veri taşıma, File Agent kök değişikliği) bu paketin kapsamında değil — ayrı, açıkça onaylanmış bir görev.
