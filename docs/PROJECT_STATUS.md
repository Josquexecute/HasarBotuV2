...
- **Kök neden (tahmin edilmedi, kodla doğrulandı):** HB-2026-197'nin "V1 aktarımı" silinmiş bir tek-seferlik script'ti, yalnız dosya TÜRÜNÜ (trafik/kasko) okuyordu — `_HASARBOTU/takip.json` içindeki notlar/görevler/sorumlu-eksper/takip tarihi/ihbar no HİÇ taşınmamıştı, bu hiçbir yerde kullanıcıya sürülmemişti.
- **Kalıcı sistem yazıldı (4 commit):** `packages/database/migrations/0046_v1_import_provenance.js` (append-only idempotency tablosu), `packages/domain/src/v1-import.ts` (saf karar çekirdeği — boşsa safe_backfill, doluysa conflict, asla kör ezme), `services/api/src/v1-import/*` (`planV1Import` 0 yazma / `applyV1Import` her klasör kendi transaction'ında + taze TOCTOU), `deploy/windows-service/run-v1-import.mjs` (CLI — varsayılan PREVIEW, `--apply` yalnız gerçek TTY + tam "UYGULA" onayıyla).
- **Geliştirme sırasında kendi test paketimle bulunup düzeltilen 2 gerçek kusur:** (1) yeni case'lerde `follow_up_date` hiç yazılmıyordu (yanlış kod yoluna bakıyordu); (2) AGENTS.md'nin " - N" soneki deseniyle aynı plakanın AYRI V1 klasörleri, apply-öncesi TOCTOU kontrolü `plate_normalized` bazlı olduğu için birbirini sahte "çakışma" sayıp ikinci case'i atlıyordu — kontrol `v1_import_records.source_relative_path` bazlı hale getirildi (plaka bu sistemde benzersiz kimlik değil).
- **Migration 0046 eklenmesi 2 önceden var olan test dosyasının migration-sayısı varsayımını kaydırdı** — ikisi de düzeltildi (mantık değişmedi): `integration.test.ts`nin var olan `HIDDEN_MIGRATION_NAMES` genişleme noktası kullanıldı; `traffic-value-loss-hardening.test.ts`nin sabit `count:3`→`4` oldu.
- **Beklenmeyen bulgu:** gerçek prod DB HEAD'i `0044`de kalmıştı — `0045_kasco_mandatory_checks` (ayrı, önceden yazılmış özellik) de hiç prod'a uygulanmamıştı. Migration sıralı çalıştığından 0046'ya ulaşmak 0045'i zorunlu kıldı; kullanıcıya ayrıca soruldu, açıkça onaylandı. İkisi de saf CREATE TABLE, mevcut hiçbir tabloya/veriye dokunmadı (`cases` satır sayısı 164, değişmedi).
- **Gerçek, salt-okunur preview** (`C:\HasarBotuStorage\BARAN GLOBAL EKSPERTİZ` + gerçek prod DB, `--apply` YOK): **202 klasör** — 140 JSON'lu, 140 TXT'li, 61 sidecar'sız (`missing_sidecar`, silinmedi/varsayılmadı), 1 ayrıştırılamayan ad. Plan: **2 yeni case, 87 mevcut case'e backfill** (hiçbirinde alan-seviyesi conflict — hepsi boş alan), 46 claimType bilinmiyor, 5 plaka-belirsiz (aynı plakada 2 V2 adayı, tahmin edilmedi), **463 yeni not, 13 yeni görev** içeri aktarılabilir durumda.
- **Gerçek tam doğrulama:** typecheck (tüm workspace) 0 hata; lint 0 hata (13 önceden var olan uyarı, değişmedi); **tam test suite gerçek `hasarbotu_test` PostgreSQL'e karşı: 1.906 test PASS, 0 başarısız** (domain 784, contracts 324, database 74, desktop-bridge 10, api 529, file-agent 110, desktop 75).
- **Apply hiç tetiklenmedi** — kullanıcının tek onayı bu adım için ayrıca bekleniyor. Production secret hiçbir yerde loglanmadı. Tam gerekçe `docs/DECISION_LOG.md` HB-2026-199'da.
- **Not (kayıt düzeltmesi):** bu işin 4 commit mesajı yanlışlıkla "HB-2026-198"e referans veriyor (o ID zaten bir önceki girişe ait) — commit amend edilmedi, gerçek takip numarası HB-2026-199'dur.
...
## Paketleme öncesi son hardening turu (2026-08-09, HB-2026-192..193)

- **npm audit:** semver-safe düzeltmeler uygulandı (`npm audit fix`, force yok) — fast-uri/nanoid/undici patch sürümlerine taşındı, ayrı commit (HB-2026-193). `pdfjs-dist` (GHSA-hq66-cqwq-w95j) **kasıtlı olarak yükseltilmedi**: gerçek, kod seviyesinde inceleme (kurulu paketin kendi `.d.ts` dosyaları) `enableScripting`in yalnız `PDFViewer`/`AnnotationLayer`de var olduğunu, `getDocument()`in (`DocumentInitParameters`) bu alanı HİÇ taşımadığını kanıtladı — derleyici doğruladı. Bu reponun 3 gerçek kullanım noktası hiçbir zaman `PDFViewer`/`AnnotationLayer` örneklemiyor (yalnız `getDocument`+`getTextContent`+`getOperatorList`+`getViewport`+temel `page.render()` + `standard_fonts` yol okuma) — CVE'nin çalışma yolu **yapısal olarak erişilemez**. pdf.js semver izlemez, API-uyum kanıtı yok; kullanıcının "güvenli kanıtlanamıyorsa değiştirme" talimatına göre dokunulmadı. **Tek kalan açık risk budur, disclosed.**
- **Pre-existing test borcu bulundu ve düzeltildi (HB-2026-192):** `packages/database/test/integration.test.ts` migration 0042'den beri güncellenmemiş; 0044 için yazılmış bir gizleme shim'i yalnız 0045'i (Kasko) bilmediği için 52/74 test cascade FAIL verdi. Shim küçük bir diziye genellendi + 2 yeni tablo adı eklendi → 74/74 PASS. Kasko'nun kendisi bu borcu YARATMADI, yalnız ilk kez görünür kıldı (dosya `git status`ta hiç değişmemişti) — ama "PASS" diyebilmek için düzeltilmesi zorunluydu.
- **Tam doğrulama (yeniden, uçtan uca):** typecheck/lint/build/check:deploy temiz; `npm test` tüm workspace'ler zincir halinde: src 383/389 (6 ortam-koşullu skip), domain 768, contracts 324, database 74, desktop-bridge 10, **api 516 (78 dosya, tüm Kasko E2E dahil — 7/7 fail-closed kapanış engeli, Trafik etkilenmiyor, closed-case yazma reddi, permission/lifecycle tutarlılığı, needs_review türetimi, audit/provenance yeniden doğrulandı)**, file-agent 110, desktop 75 — **hepsi PASS, 0 başarısız.**
- **API dizini ACL — kullanıcı onayıyla UYGULANDI:** taze preview drift göstermedi, kullanıcının tek açık onayıyla gerçek `-Apply` çalıştırıldı. Bağımsız `Get-Acl` sorgusuyla doğrulandı: root'ta tam 2 ACE (SYSTEM:ReadAndExecute, Administrators:FullControl), `logs\`de tam 2 ACE (SYSTEM:Modify, Administrators:FullControl), eski `Authenticated Users`/`Users` grantları tamamen yok, `hasarbotu-api` servisi hâlâ Running/Automatic (dokunulmadı). **Faz 1'in iki kayıtlı artık riskinden ikincisi de kapandı.**
- Commit: HB-2026-192 (database test shim), HB-2026-193 (npm audit fix), HB-2026-194 (ACL apply kaydı). Gerçek production verisi mutate edilmedi; production servisinin dizin ACL'si kullanıcı onayıyla gerçekten değişti (planlanan, önizlenen, doğrulanan değişiklik).
...
- **Tek gerçek engelleyici bulundu ve KAPATILDI:** `storage_roots` tablosunda gerçek `baran-global` organizasyonu için 0 satır — hiçbir INSERT yolu yoktu (B9/admin-bootstrap ile aynı sınıf boşluk, bir katman daha derinde). İlk gerçek vakanın workspace provisioning/konum atama/dosya işlemi bu yüzden ilk adımda fail-closed reddedilecekti. `deploy/windows-service/bootstrap-storage-root.mjs` yazıldı (+17 test, gerçek `hasarbotu_test` DB'sinde 17/17 PASS), gerçek production DB'ye karşı salt-okunur önizleme `Status:"ready"` doğrulandı, **kullanıcının tek onayıyla gerçek `--apply` çalıştırıldı ve bağımsızca doğrulandı** — `storage_roots` artık 1 satır (`baran-global` / `baran-global-primary`), audit kaydı mevcut, başka hiçbir tablo etkilenmedi.
- Ayrıca 1 kozmetik hata (WinSW XML render'inde Türkçe mojibake, `install-services.ps1`) ve 2 ilgisiz gerçek test/doküman hatası (policy-ai.test.ts tarih varsayımı; D9 rollback runbook'unun gerçekte var olmayan parametreler kullanması) bulunup düzeltildi, gerçek makinede/testte doğrulandı.
- Deployed API+File Agent `dist/` HEAD'in taze build'iyle **byte-birebir eşleşti** (435/435 + 60/60 dosya) — stale deploy yok. `npm run typecheck`/`lint`/`build`/`check:deploy`/`test` (2234 test) ve `deploy/windows-service/`nin tüm `.tests.ps1`(13)/`.test.mjs`(15) suite'leri gerçek makinede yeniden koşturuldu — hepsi PASS.
- Kayda geçen, düzeltilmeyen artık riskler: `hasarbotu-api` LocalSystem altında + dizini File Agent'ın aksine özel ACL almadı (tutarsızlık, tek-operatör makinede düşük pratik risk); `apply-file-agent-service-vehicle-probe.ps1 -Rollback`ın artık gerçekten Running olan servise karşı bir "dokunma" koruması yok.
