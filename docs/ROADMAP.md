...
| v0.7 — İşçilik AI | Excel şablon profilleri, AI öneri, eksper onayı, öğrenme sözlüğü, güvenli Excel yazımı | **Kullanıcı kontrollü çekirdek tamamlandı:** sürümlü parça/işçilik föyü, satır kalemleri, toplam, RBAC/idempotency/audit ve gerçek Dosya Detayı İşçilik alanı mevcut; AI öneri, Excel şablonu ve güvenli Excel yazımı bekliyor |
...
Paket 43 kullanıcı kontrollü İşçilik çekirdeği (v0.7’nin ilk dilimi) uygulanmaktadır (karar: HB-2026-049). Tam kalite, gerçek PostgreSQL/API, gerçek Chrome/CDP ve repository dışı fresh checkout kapıları geçmeden sonraki dilime başlanmaz. Sonraki mantıklı dilimler: İşçilik AI önerisi ve güvenli Excel yazımı (kritik işlem modeli) ile — ayrı güvenlik/deployment kararı gerektiren — v0.6 Gmail OAuth/API + gelen e-posta senkronizasyonu.
