...
const candidateIdSchema=z.string().regex(/^[a-zA-Z0-9_.-]{1,80}$/)
...
export const policyAiUsageItemSchema=z.strictObject({runId:idSchema,caseId:caseIdSchema,providerId:z.enum(POLICY_AI_PROVIDER_IDS),modelId:providerFactSchema,inputCharacters:z.number().int().nonnegative(),outputCharacters:z.number().int().nonnegative(),estimatedCostMinor:z.number().int().nonnegative(),actualCostMinor:z.number().int().nonnegative().nullable(),status:z.enum(['provider_disabled','budget_blocked','completed','failed','cancelled']),safeErrorCode:safeCodeSchema.nullable(),startedAt:utcDateTimeSchema,completedAt:utcDateTimeSchema.nullable()})
