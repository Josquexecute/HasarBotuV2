...
describeDb('PostgreSQL entegrasyonu (gercek veritabani)', () => {
  ...
  it('bos veritabanina ileri migration deterministik uygulanir', async () => {
    ...
    expect(applied.map((m) => m.name)).toEqual([
      ...
      '0026_email_draft_core',
      '0027_email_ai_suggestions',
      '0028_labor_sheet_core',
      '0029_labor_ai_suggestions',
      '0030_pert_assessment_core',
      '0031_labor_allocation_ai',
      '0032_ai_evidence_enrichment',
      '0033_expert_baseline_evidence',
      '0034_labor_allocation_application',
      '0035_labor_excel_profiles',
      '0036_labor_allocation_progress',
      '0037_labor_excel_profile_selection',
      '0038_labor_excel_profile_categories',
      '0039_labor_allocation_schema_v2',
      '0040_labor_category_persistence',
      '0041_labor_category_comparison',
      '0042_case_inventory',
      '0043_traffic_value_loss_real_market_revision',
    ])
    ...
    expect(tables.rows.map((r: { table_name: string }) => r.table_name)).toEqual([
      ...
      'case_task_events',
      'case_tasks',
      'case_vehicle_owner_sets',
      'case_vehicle_owners',
      'case_vehicle_profile_versions',
  ...
  it('0042 araç sahibi listesi tenant, biçim ve append-only kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const caseId = uuidv7()
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p-inv-db','Inventory DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'inv-db@test.local','x','Inventory DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,4201,'2026/4201','traffic','reporting','34 DB 420','34DB420')`,
    ...
    await pool.query(
      `INSERT INTO case_vehicle_owners (id,organization_id,case_id,set_version,ordinal,name,phone,created_by_user_id)
       VALUES ($1,$2,$3,1,0,'Ahmet Yılmaz','05321234567',$4)`,
      [ownerId, organizationId, caseId, userId],
    )
    await pool.query(
    ...
    await expect(pool.query(
      ...
      [uuidv7(), organizationId, caseId, userId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'case_vehicle_owners_valid' })

    // Harf içeren telefon reddedilir (yalnız rakam/boşluk/yaygın ayraç kabul edilir).
    await expect(pool.query(
      `INSERT INTO case_vehicle_owners (id,organization_id,case_id,set_version,ordinal,name,phone,created_by_user_id)
       VALUES ($1,$2,$3,1,1,'Kötü Telefon','call-me',$4)`,
      [uuidv7(), organizationId, caseId, userId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'case_vehicle_owners_valid' })

    // Aynı (org,case,set_version,ordinal) ikinci kez yazılamaz.
    await expect(pool.query(
      `INSERT INTO case_vehicle_owners (id,organization_id,case_id,set_version,ordinal,name,phone,created_by_user_id)
       VALUES ($1,$2,$3,1,0,'Başka İsim',NULL,$4)`,
      [uuidv7(), organizationId, caseId, userId],
    )).rejects.toMatchObject({ code: '23505', constraint: 'case_vehicle_owners_set_unique' })

    // Satırlar append-only'dir.
    await expect(pool.query("UPDATE case_vehicle_owners SET name='Değişti' WHERE id=$1", [ownerId]))
      .rejects.toMatchObject({ code: '23001' })
    await expect(pool.query('DELETE FROM case_vehicle_owners WHERE id=$1', [ownerId]))
      .rejects.toMatchObject({ code: '23001' })

    // Güncel okuma yalnız işaretlenen set_version'ı görür.
    const current = await pool.query(
      `SELECT name,phone FROM case_vehicle_owners
        WHERE organization_id=$1 AND case_id=$2
          AND set_version=(SELECT current_set_version FROM case_vehicle_owner_sets WHERE organization_id=$1 AND case_id=$2)
        ORDER BY ordinal`,
      [organizationId, caseId],
    )
    expect(current.rows).toEqual([{ name: 'Ahmet Yılmaz', phone: '05321234567' }])
  })
  ...
  it('0043 yeni değer kaybı rule allowlist migrationını geri alır ve yeniden uygular', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name))
      .toEqual(['0043_traffic_value_loss_real_market_revision'])
    const oldDefinition = await pool.query(
      `SELECT pg_get_constraintdef(oid) AS def
       FROM pg_constraint
       WHERE conname='traffic_value_loss_versions_rule_locked'`,
    )
  ...
  it('0036 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress'])
    const removed = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.columns
        WHERE table_name='labor_allocation_runs' AND column_name='total_chunk_count'`,
    )
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })

  it('0036 ilerleme durumlarını ve geçiş kurallarını DB seviyesinde korur', async () => {
    // Yeni durumlar kabul edilir.
    const statuses = await pool.query(
      `SELECT pg_get_constraintdef(oid) AS def FROM pg_constraint
        WHERE conname='labor_allocation_runs_valid'`,
    )
    const definition = String(statuses.rows[0].def)
    for (const status of ['queued', 'cancel_requested', 'cancelled']) {
      expect(definition).toContain(status)
    }
    // İş bölünmesi alanları birlikte yazılır.
    expect(definition).toContain('total_chunk_count')

    // Aktif run araması için kısmi indeks vardır.
    const index = await pool.query(
      "SELECT count(*)::int AS n FROM pg_indexes WHERE indexname='labor_allocation_runs_active_idx'",
    )
    expect(index.rows).toEqual([{ n: 1 }])
  })

  it('0035 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 9, quiet: true })
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles'])
    const removed = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.tables
        WHERE table_name='labor_excel_profiles'`,
    )
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })

  it('0035 eşlemenin kanonik tür kümesini tam kapsamasını zorlar', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const profileId = uuidv7()
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p60db','P60 DB')", [organizationId])
    await pool.query(
      `INSERT INTO users (id,organization_id,email,display_name,password_hash)
       VALUES ($1,$2,'p60db@test.local','P60 DB','x')`,
      [userId, organizationId],
    )
    await pool.query(
      'INSERT INTO labor_excel_profiles (id,organization_id,created_by_user_id) VALUES ($1,$2,$3)',
      [profileId, organizationId, userId],
    )
    const columns = JSON.stringify([{ key: 'ISCILIK', label: 'İşçilik' }])
    const complete = JSON.stringify({
      repair: 'ISCILIK', replace: null, remove_install: null, paint: null,
      consumable: null, calibration: null, related_operation: null, other: null,
    })
    const partial = JSON.stringify({ repair: 'ISCILIK', replace: null })

    // Eksik anahtarlı eşleme DB seviyesinde reddedilir.
    await expect(pool.query(
      `INSERT INTO labor_excel_profile_versions
         (id,organization_id,profile_id,profile_version,schema_version,name,columns,mapping,created_by_user_id)
       VALUES ($1,$2,$3,1,'labor-excel-profile/1.0.0','Eksik',$4::jsonb,$5::jsonb,$6)`,
      [uuidv7(), organizationId, profileId, columns, partial, userId],
    )).rejects.toMatchObject({ code: '23514' })

    // Tam eşleme kabul edilir.
    const versionId = uuidv7()
    await pool.query(
      `INSERT INTO labor_excel_profile_versions
         (id,organization_id,profile_id,profile_version,schema_version,name,columns,mapping,created_by_user_id)
       VALUES ($1,$2,$3,1,'labor-excel-profile/1.0.0','Sentetik',$4::jsonb,$5::jsonb,$6)`,
      [versionId, organizationId, profileId, columns, complete, userId],
    )

    // Sürüm immutable'dır.
    await expect(pool.query(
      "UPDATE labor_excel_profile_versions SET name='Degisti' WHERE id=$1",
      [versionId],
    )).rejects.toMatchObject({ code: '23001' })
  })
  ...
  it('0034 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application'])
    const removed = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.tables
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0033 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application', '0033_expert_baseline_evidence'])
    const removed = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.columns
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })

  it('0033 baseline alanlarını tutarlılık kurallarıyla korur', async () => {
    // Baseline alanları ya tamamen dolu ya tamamen boştur; "baseline yok ama
    // çelişki var" kaydı DB seviyesinde imkânsızdır.
    const partial = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.check_constraints
        WHERE constraint_name='labor_allocation_line_suggestions_baseline_valid'`,
    )
    expect(partial.rows).toEqual([{ n: 1 }])
    const runConstraint = await pool.query(
      `SELECT count(*)::int AS n FROM information_schema.check_constraints
        WHERE constraint_name='labor_allocation_runs_baseline_valid'`,
    )
    expect(runConstraint.rows).toEqual([{ n: 1 }])
  })
  ...
  it('0032 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0032 araç profilini sürümler, immutable tutar ve kanıt alanlarını doğrular', async () => {
    ...
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p56-db','P56 DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'p56-db@test.local','x','P56 DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,5601,'2026/5601','traffic','reporting','34 DB 561','34DB561')`,
      [caseId, organizationId],
    )
    await pool.query(
      'INSERT INTO case_vehicle_profiles (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
      [profileId, organizationId, caseId, userId],
    )
    await pool.query(
      `INSERT INTO case_vehicle_profile_versions
         (id,organization_id,case_id,profile_id,profile_version,schema_version,
          brand,model,model_year,vehicle_class,chassis_prefix,evidence_source,created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'case-vehicle-profile/1.0.0','Renault','Clio',2021,
               'passenger_car','VF1RJA00','registration_document',$5)`,
      [firstVersionId, organizationId, caseId, profileId, userId],
    )
    await pool.query('UPDATE case_vehicle_profiles SET current_version_id=$2 WHERE id=$1', [profileId, firstVersionId])

    // Tam şasi numarası (17 hane) reddedilir.
    await expect(pool.query(
      `INSERT INTO case_vehicle_profile_versions
         (id,organization_id,case_id,profile_id,profile_version,previous_version_id,schema_version,
          brand,model,model_year,vehicle_class,chassis_prefix,evidence_source,revision_reason,created_by_user_id)
       VALUES ($1,$2,$3,$4,2,$5,'case-vehicle-profile/1.0.0','Renault','Clio',2021,
               'passenger_car','VF1RJA00567123456','registration_document','Duzeltme',$6)`,
      [uuidv7(), organizationId, caseId, profileId, firstVersionId, userId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'case_vehicle_profile_versions_valid' })

    // Sürümler append-only'dir ve aggregate silinemez.
    await expect(pool.query("UPDATE case_vehicle_profile_versions SET brand='X' WHERE id=$1", [firstVersionId]))
      .rejects.toMatchObject({ code: '23001' })
    await expect(pool.query('DELETE FROM case_vehicle_profiles WHERE id=$1', [profileId]))
      .rejects.toMatchObject({ code: '23001' })
  })
  ...
  it('0031 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application', '0033_expert_baseline_evidence', '0032_ai_evidence_enrichment', '0031_labor_allocation_ai'])
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0030 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application', '0033_expert_baseline_evidence', '0032_ai_evidence_enrichment', '0031_labor_allocation_ai', '0030_pert_assessment_core'])
    const removed = await pool.query("SELECT count(*)::int AS n FROM information_schema.tables WHERE table_name='pert_assessments'")
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0030_pert_assessment_core', '0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0029 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0029_labor_ai_suggestions', '0030_pert_assessment_core', '0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0028 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0028_labor_sheet_core', '0029_labor_ai_suggestions', '0030_pert_assessment_core', '0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0027 geri alınabilir ve yeniden ileri uygulanabilir', async () => {
    ...
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application', '0033_expert_baseline_evidence', '0032_ai_evidence_enrichment', '0031_labor_allocation_ai', '0030_pert_assessment_core', '0029_labor_ai_suggestions', '0028_labor_sheet_core', '0027_email_ai_suggestions'])
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0027_email_ai_suggestions', '0028_labor_sheet_core', '0029_labor_ai_suggestions', '0030_pert_assessment_core', '0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0026 geri alınabilir ve 0027 ile yeniden ileri uygulanabilir', async () => {
    ...
    expect(removed.rows).toEqual([{ n: 0 }])
    const reapplied = await runMigrations({ databaseUrl: config.url, quiet: true })
    expect(reapplied.map((migration) => migration.name)).toEqual(['0026_email_draft_core', '0027_email_ai_suggestions', '0028_labor_sheet_core', '0029_labor_ai_suggestions', '0030_pert_assessment_core', '0031_labor_allocation_ai', '0032_ai_evidence_enrichment', '0033_expert_baseline_evidence', '0034_labor_allocation_application', '0035_labor_excel_profiles', '0036_labor_allocation_progress', '0037_labor_excel_profile_selection', '0038_labor_excel_profile_categories', '0039_labor_allocation_schema_v2', '0040_labor_category_persistence', '0041_labor_category_comparison', '0042_case_inventory', '0043_traffic_value_loss_real_market_revision'])
  })
  ...
  it('0022 geri alınabilir ve sonraki migrationlarla yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 22, quiet: true })
  ...
  it('0021 geri alınabilir ve sonraki migrationlarla yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 23, quiet: true })
  ...
  it('0020 geri alınabilir ve sonraki migrationlarla yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 24, quiet: true })
    expect(rolledBack.map((migration) => migration.name)).toEqual(['0043_traffic_value_loss_real_market_revision', '0042_case_inventory', '0041_labor_category_comparison', '0040_labor_category_persistence', '0039_labor_allocation_schema_v2', '0038_labor_excel_profile_categories', '0037_labor_excel_profile_selection', '0036_labor_allocation_progress', '0035_labor_excel_profiles', '0034_labor_allocation_application', '0033_expert_baseline_evidence', '0032_ai_evidence_enrichment', '0031_labor_allocation_ai', '0030_pert_assessment_core', '0029_labor_ai_suggestions', '0028_labor_sheet_core', '0027_email_ai_suggestions', '0026_email_draft_core', '0025_closure_fees_reports', '0024_case_notes_tasks', '0023_traffic_value_loss_reports', '0022_traffic_value_loss_core', '0021_policy_ai_provider_recovery', '0020_real_policy_ai_provider'])
  ...
  it('0019 geri alınabilir ve sonraki migrationlarla yeniden uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 25, quiet: true })
  ...
  it('0018 geri alınabilir ve sonraki migrationlarla yeniden uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 26, quiet: true })
  ...
  it('0017 geri alınabilir ve sonraki migrationlarla yeniden uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 27, quiet: true })
  ...
  it('0015 geri alınabilir ve sonraki migrationlarla yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 29, quiet: true })
  ...
  it('0014 geri alinabilir, eski servis profilini donusturur ve yeniden ileri uygulanabilir', async () => {
    const rolledBack = await runMigrations({ databaseUrl: config.url, direction: 'down', count: 30, quiet: true })
  ...
  it('0030 tek değerlendirme, immutable sürüm ve kanaat/merkez tutarlılık kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const caseId = uuidv7()
    const assessmentId = uuidv7()
    const versionId = uuidv7()
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p45-db','P45 DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'p45-db@test.local','x','P45 DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,4501,'2026/4501','traffic','reporting','34 DB 451','34DB451')`,
      [caseId, organizationId],
    )
    await pool.query(
    ...
    await pool.query(
      `INSERT INTO pert_assessment_versions
         (id,organization_id,case_id,assessment_id,assessment_version,workflow_status,
          estimated_damage_minor,market_value_minor,source_type,currency,created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'under_review',480000000,625000000,'user_entered','TRY',$5)`,
      [versionId, organizationId, caseId, assessmentId, userId],
    )
    await pool.query('UPDATE pert_assessments SET current_version_id=$2 WHERE id=$1', [assessmentId, versionId])

    // Case başına yalnız tek değerlendirme.
    await expect(pool.query(
      'INSERT INTO pert_assessments (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
      [uuidv7(), organizationId, caseId, userId],
    )).rejects.toMatchObject({ code: '23505', constraint: 'pert_assessments_case_unique' })

    // Kanaat gerekçesiz olamaz; karar durumu merkez kararıyla birebir eşleşmelidir.
    const invalidVersion = (fields: string) => pool.query(
      `INSERT INTO pert_assessment_versions
         (id,organization_id,case_id,assessment_id,assessment_version,previous_version_id,
          workflow_status,expert_opinion,expert_rationale,center_decision,
          source_type,currency,revision_reason,created_by_user_id)
       VALUES ($1,$2,$3,$4,2,$5,${fields},'manual_revision','TRY','Gerekçe',$6)`,
      [uuidv7(), organizationId, caseId, assessmentId, versionId, userId],
    )
    await expect(invalidVersion("'expert_opinion_issued','pert',NULL,NULL"))
      .rejects.toMatchObject({ code: '23514', constraint: 'pert_assessment_versions_decision_valid' })
    await expect(invalidVersion("'repair_decided','pert','Gerekçeli','pert'"))
      .rejects.toMatchObject({ code: '23514', constraint: 'pert_assessment_versions_decision_valid' })

    // Sürümler append-only'dir ve aggregate silinemez.
    await expect(pool.query("UPDATE pert_assessment_versions SET workflow_status='pert_candidate' WHERE id=$1", [versionId]))
      .rejects.toMatchObject({ code: '23001' })
    await expect(pool.query('DELETE FROM pert_assessments WHERE id=$1', [assessmentId]))
      .rejects.toMatchObject({ code: '23001' })
  })

  it('0031 dağıtım önerisini ayrı aggregate, append-only ve sürüm zinciriyle korur', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const caseId = uuidv7()
    ...
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p54-db','P54 DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'p54-db@test.local','x','P54 DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,5401,'2026/5401','traffic','reporting','34 DB 541','34DB541')`,
      [caseId, organizationId],
    )
    await pool.query(
      'INSERT INTO labor_sheets (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
      [sheetId, organizationId, caseId, userId],
    )

    const insertRun = (id: string, planHash: string) => pool.query(
      `INSERT INTO labor_allocation_runs
         (id,organization_id,case_id,source_sheet_id,source_sheet_version,evidence_hash,plan_hash,
          ...
          outbound_input_characters,provider_retention_mode,pricing_version,estimated_cost_minor,
          created_by_user_id,started_at)
       VALUES ($1,$2,$3,$4,1,$5,$6,'deterministic-success','1.0.0','deterministic',
               'labor-allocation-ai/1.0.0','labor-allocation-suggestion/1.0.0',
               'labor-operation-types/1.0.0','labor-allocation-rules/1.0.0',false,
               'labor-allocation-pii/local-only',1200,'local_only','pricing/1.0.0',0,$7,now())`,
      [id, organizationId, caseId, sheetId, hash, planHash, userId],
    )
    await insertRun(runId, hash)

    const insertLine = (ordinal: number) => pool.query(
      `INSERT INTO labor_allocation_line_suggestions
         (id,organization_id,case_id,run_id,line_ordinal,source_description,source_action,
          ...
          reasoning,confidence,control_required)
       VALUES ($1,$2,$3,$4,$5,'Ön tampon','Onarım',0,1000000,
               '[{"operationType":"repair","amountMinor":1000000}]'::jsonb,'repair_indicated',
               '{"repair_labor":1000000,"new_part_or_ownership":0,"remove_install":0,
                 "paint_and_consumable":0,"calibration":0,"related_operations":0}'::jsonb,
               1000000,0,'Onarım uygun.','Gerekçe.',0.8000,false)`,
      [uuidv7(), organizationId, caseId, runId, ordinal],
    )
    await insertLine(1)

    // Satır sonuçları immutable'dır.
    await expect(pool.query('UPDATE labor_allocation_line_suggestions SET confidence=0.1 WHERE run_id=$1', [runId]))
      .rejects.toMatchObject({ code: '23001' })
    await expect(pool.query('DELETE FROM labor_allocation_line_suggestions WHERE run_id=$1', [runId]))
      .rejects.toMatchObject({ code: '23001' })

    // Bildirilen satır sayısı gerçek satırlarla uyuşmalıdır.
    const finalize = (lineCount: number) => pool.query(
      `UPDATE labor_allocation_runs
          SET status='review_required',line_count=$2,control_required_count=0,
              completed_at=now(),version=version+1
        WHERE id=$1`,
      [runId, lineCount],
    )
    await expect(finalize(5)).rejects.toMatchObject({ code: '23514' })
    await finalize(1)

    // Terminal run immutable'dır ve silinemez; run'a yeni satır eklenemez.
    await expect(finalize(1)).rejects.toMatchObject({ code: '23001' })
    await expect(pool.query('DELETE FROM labor_allocation_runs WHERE id=$1', [runId]))
      .rejects.toMatchObject({ code: '23001' })
    await expect(insertLine(2)).rejects.toMatchObject({ code: '23001' })

    // Aynı analiz anahtarı için İKİNCİ BAŞARILI sonuç üretilemez (idempotency).
    // Başarısız çalıştırmalar yeniden denenebilir olmalıdır, bu yüzden koruma
    // yalnız `review_required` satırları üzerindedir.
    const retryId = uuidv7()
    await insertRun(retryId, hash)
    await pool.query(
      `INSERT INTO labor_allocation_line_suggestions
         (id,organization_id,case_id,run_id,line_ordinal,source_description,source_action,
          source_part_amount_minor,source_labor_amount_minor,allocations,repair_replace_opinion,
          economic_buckets,economic_repair_total_minor,economic_replace_total_minor,economic_note,
          reasoning,confidence,control_required)
       VALUES ($1,$2,$3,$4,1,'Ön tampon','Onarım',0,1000000,
               '[{"operationType":"repair","amountMinor":1000000}]'::jsonb,'repair_indicated',
               '{"repair_labor":1000000,"new_part_or_ownership":0,"remove_install":0,
                 "paint_and_consumable":0,"calibration":0,"related_operations":0}'::jsonb,
               1000000,0,'Onarım uygun.','Gerekçe.',0.8000,false)`,
      [uuidv7(), organizationId, caseId, retryId],
    )
    await expect(pool.query(
      `UPDATE labor_allocation_runs
          SET status='review_required',line_count=1,control_required_count=0,
              completed_at=now(),version=version+1
        WHERE id=$1`,
      [retryId],
    )).rejects.toMatchObject({ code: '23505', constraint: 'labor_allocation_runs_identity_unique' })

    // Usage ledger `labor_allocation` modülü yalnız kendi run referansını taşır.
    const insertLedger = (columns: string, values: string, params: unknown[]) => pool.query(
      `INSERT INTO ai_usage_ledger
         (id,organization_id,case_id,usage_module,${columns},provider_id,model_id,
          request_hash,input_characters,output_characters,estimated_cost_minor,status,started_at)
       VALUES ($1,$2,$3,'labor_allocation',${values},'deterministic-success','deterministic',
               $${4 + params.length},10,10,0,'completed',now())`,
      [uuidv7(), organizationId, caseId, ...params, hash],
    )
    await insertLedger('labor_allocation_run_id', '$4', [runId])
    // Modül kendi referansı olmadan yazılamaz.
    await expect(insertLedger('run_id', '$4', [runId]))
      .rejects.toMatchObject({ code: '23514', constraint: 'ai_usage_ledger_valid' })
  })

  it('0029 labor AI policy, immutable run, provenance ve ortak usage ledger kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const caseId = uuidv7()
    const runId = uuidv7()
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p44-db','P44 DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'p44-db@test.local','x','P44 DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,4401,'2026/4401','traffic','reporting','34 DB 441','34DB441')`,
      [caseId, organizationId],
    )
    await pool.query(
      `INSERT INTO ai_provider_policies (id,organization_id,monthly_budget_minor,per_request_budget_minor)
       VALUES ($1,$2,1000,100)`,
      [uuidv7(), organizationId],
    )
    const policy = await pool.query(
      'SELECT labor_enabled,labor_allowed_provider_ids FROM ai_provider_policies WHERE organization_id=$1',
      [organizationId],
    )
    expect(policy.rows).toEqual([{ labor_enabled: false, labor_allowed_provider_ids: [] }])
    await expect(pool.query(
      `UPDATE ai_provider_policies SET labor_allowed_provider_ids=ARRAY['unsupported-provider']::text[]
        WHERE organization_id=$1`,
      [organizationId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'ai_provider_policies_labor_provider_valid' })

    await pool.query(
      `INSERT INTO labor_ai_suggestion_runs
         (id,organization_id,case_id,base_sheet_version,plan_hash,provider_id,provider_version,model_id,
          prompt_template_version,output_schema_version,status,external_provider,privacy_policy_version,
          outbound_input_characters,provider_retention_mode,pricing_version,estimated_cost_minor,
          safe_error_code,created_by_user_id,completed_at)
       VALUES ($1,$2,$3,NULL,$4,'deterministic-success','deterministic-labor/1.0.0','local-labor-fixture-v1',
               'labor-ai-draft/1.0.0','labor-ai-suggestion/1.0.0','provider_disabled',false,'labor-ai-pii/local-only',
               10,'local_only','deterministic-cost/1.0.0',1,'AI_PROVIDER_DISABLED',$5,now())`,
      [runId, organizationId, caseId, 'a'.repeat(64), userId],
    )
    // Terminal run immutable'dır.
    await expect(pool.query("UPDATE labor_ai_suggestion_runs SET status='failed' WHERE id=$1", [runId]))
      .rejects.toMatchObject({ code: '23001' })

    // ai_assisted sürüm run kimliği olmadan kabul edilmez.
    const sheetId = uuidv7()
    await pool.query(
      'INSERT INTO labor_sheets (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
      [sheetId, organizationId, caseId, userId],
    ...
    await expect(pool.query(
      `INSERT INTO labor_sheet_versions
         (id,organization_id,case_id,sheet_id,sheet_version,source_type,currency,labor_ai_suggestion_run_id,created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'ai_assisted','TRY',NULL,$5)`,
      [uuidv7(), organizationId, caseId, sheetId, userId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'labor_sheet_versions_shape_valid' })

    // labor_sheet usage kaydı run kimliği olmadan kabul edilmez.
    await expect(pool.query(
      `INSERT INTO ai_usage_ledger
         (id,organization_id,case_id,usage_module,provider_id,model_id,request_hash,
          input_characters,estimated_cost_minor,status,started_at)
       VALUES ($1,$2,$3,'labor_sheet','deterministic-success','local-labor-fixture-v1',$4,10,1,'provider_disabled',now())`,
      [uuidv7(), organizationId, caseId, 'b'.repeat(64)],
    )).rejects.toMatchObject({ code: '23514', constraint: 'ai_usage_ledger_valid' })
  })

  it('0028 tek föy, immutable sürüm, sürüm zinciri ve satır kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    const userId = uuidv7()
    const caseId = uuidv7()
    const sheetId = uuidv7()
    const versionId = uuidv7()
    await pool.query("INSERT INTO organizations (id,code,name) VALUES ($1,'p43-db','P43 DB')", [organizationId])
    await pool.query(
      "INSERT INTO users (id,organization_id,email,password_hash,display_name) VALUES ($1,$2,'p43-db@test.local','x','P43 DB')",
      [userId, organizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,workflow_stage,plate,plate_normalized)
       VALUES ($1,$2,2026,4301,'2026/4301','traffic','reporting','34 DB 431','34DB431')`,
      [caseId, organizationId],
    )
    await pool.query(
      'INSERT INTO labor_sheets (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
    ...
    await pool.query(
      ...
      [versionId, organizationId, caseId, sheetId, userId],
    )
    await pool.query(
      `INSERT INTO labor_sheet_items
         (id,organization_id,case_id,sheet_id,sheet_version_id,ordinal,description,action,part_amount_minor,labor_amount_minor)
       VALUES ($1,$2,$3,$4,$5,1,'Ön tampon','Değişim',1840000,220000)`,
      [uuidv7(), organizationId, caseId, sheetId, versionId],
    )
    await pool.query('UPDATE labor_sheets SET current_version_id=$2 WHERE id=$1', [sheetId, versionId])

    // Her iki tutarı sıfır olan satır DB seviyesinde reddedilir.
    await expect(pool.query(
      `INSERT INTO labor_sheet_items
         (id,organization_id,case_id,sheet_id,sheet_version_id,ordinal,description,action,part_amount_minor,labor_amount_minor)
       VALUES ($1,$2,$3,$4,$5,2,'Boş kalem','Ölçüm',0,0)`,
      [uuidv7(), organizationId, caseId, sheetId, versionId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'labor_sheet_items_shape_valid' })

    // Case başına yalnız tek föy.
    await expect(pool.query(
      'INSERT INTO labor_sheets (id,organization_id,case_id,created_by_user_id) VALUES ($1,$2,$3,$4)',
      [uuidv7(), organizationId, caseId, userId],
    )).rejects.toMatchObject({ code: '23505', constraint: 'labor_sheets_case_unique' })

    // Sürüm ve satır append-only'dir.
    await expect(pool.query("UPDATE labor_sheet_versions SET source_type='manual_revision' WHERE id=$1", [versionId]))
      .rejects.toMatchObject({ code: '23001' })

    // Yanlış previous ile ikinci sürüm sürüm zinciri guard'ıyla reddedilir.
    await expect(pool.query(
      `INSERT INTO labor_sheet_versions
         (id,organization_id,case_id,sheet_id,sheet_version,previous_version_id,source_type,currency,revision_reason,created_by_user_id)
       VALUES ($1,$2,$3,$4,2,NULL,'manual_revision','TRY','Gerekçe',$5)`,
      [uuidv7(), organizationId, caseId, sheetId, userId],
    )).rejects.toMatchObject({ code: '23514' })

    // Aggregate silinemez.
    await expect(pool.query('DELETE FROM labor_sheets WHERE id=$1', [sheetId]))
      .rejects.toMatchObject({ code: '23001' })
  })

  it('0027 tenant, email provider policy, immutable run ve ortak usage ledger kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    ...
    await pool.query(
      `INSERT INTO organizations (id,code,name)
       VALUES ($1,'p42-db','P42 DB'),($2,'p42-db-foreign','P42 DB Foreign')`,
      [organizationId, foreignOrganizationId],
    )
    await pool.query(
      `INSERT INTO users (id,organization_id,email,password_hash,display_name)
       VALUES ($1,$3,'p42-db@test.local','x','P42 DB'),
              ($2,$4,'p42-db-foreign@test.local','x','P42 DB Foreign')`,
    ...
    await pool.query(
      `INSERT INTO cases
       ...
       VALUES ($1,$2,2026,4201,'2026/4201','traffic','reporting','34 DB 421','34DB421')`,
      [caseId, organizationId],
    )
    await pool.query(
      `INSERT INTO ai_provider_policies
         (id,organization_id,monthly_budget_minor,per_request_budget_minor)
       VALUES ($1,$2,1000,100)`,
      [uuidv7(), organizationId],
    )
    const policy = await pool.query(
      `SELECT email_enabled,email_allowed_provider_ids
         FROM ai_provider_policies WHERE organization_id=$1`,
      [organizationId],
    )
    expect(policy.rows).toEqual([{ email_enabled: false, email_allowed_provider_ids: [] }])
    await expect(pool.query(
      `UPDATE ai_provider_policies
          SET email_allowed_provider_ids=ARRAY['unsupported-provider']::text[]
        WHERE organization_id=$1`,
      [organizationId],
    )).rejects.toMatchObject({
      code: '23514',
      constraint: 'ai_provider_policies_email_provider_valid',
    })
    await pool.query(
      `INSERT INTO email_ai_suggestion_runs
         (id,organization_id,case_id,draft_type,base_preview_hash,plan_hash,
          provider_id,provider_version,model_id,prompt_template_version,output_schema_version,
          status,external_provider,privacy_policy_version,outbound_input_characters,
          provider_retention_mode,pricing_version,estimated_cost_minor,safe_error_code,
          created_by_user_id,completed_at)
       VALUES ($1,$2,$3,'case_status_update',$4,$5,'deterministic-success',
               'deterministic-email/1.0.0','local-email-fixture-v1',
               'email-ai-draft/1.0.0','email-ai-suggestion/1.0.0',
               'provider_disabled',false,'email-ai-pii/local-only',10,
               'local_only','deterministic-cost/1.0.0',1,'AI_PROVIDER_DISABLED',$6,now())`,
      [runId, organizationId, caseId, 'a'.repeat(64), 'b'.repeat(64), userId],
    )
    const draftId = uuidv7()
    await pool.query(
      `INSERT INTO email_drafts
         (id,organization_id,case_id,draft_type,created_by_user_id)
       VALUES ($1,$2,$3,'case_status_update',$4)`,
      [draftId, organizationId, caseId, userId],
    )
    await expect(pool.query(
      `INSERT INTO email_draft_versions
         (id,organization_id,case_id,draft_id,draft_version,subject,body,
          ...
          created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'Sentetik','Sentetik gövde',
               'email-draft-template/1.0.0','ai_assisted',NULL,$5,$6)`,
      [uuidv7(), organizationId, caseId, draftId, '9'.repeat(64), userId],
    )).rejects.toMatchObject({
      code: '23514',
      constraint: 'email_draft_versions_shape_valid',
    })
    await expect(pool.query(
      `INSERT INTO email_draft_versions
         (id,organization_id,case_id,draft_id,draft_version,subject,body,
          ...
          created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'Sentetik','Sentetik gövde',
               'email-draft-template/1.0.0','deterministic_template',$5,$6,$7)`,
      [uuidv7(), organizationId, caseId, draftId, runId, '8'.repeat(64), userId],
    )).rejects.toMatchObject({
      code: '23514',
      constraint: 'email_draft_versions_shape_valid',
    })
    await pool.query(
      `INSERT INTO ai_usage_ledger
         (id,organization_id,case_id,run_id,email_suggestion_run_id,usage_module,
          ...
          estimated_cost_minor,status,safe_error_code,pricing_version,started_at,completed_at)
       VALUES ($1,$2,$3,NULL,$4,'email_draft','deterministic-success',
               'local-email-fixture-v1',$5,10,0,1,'provider_disabled',
               'AI_PROVIDER_DISABLED','deterministic-cost/1.0.0',now(),now())`,
      [uuidv7(), organizationId, caseId, runId, 'c'.repeat(64)],
    )
    await expect(pool.query(
      `INSERT INTO email_ai_suggestion_runs
         (id,organization_id,case_id,draft_type,base_preview_hash,plan_hash,
       ...
       VALUES ($1,$2,$3,'case_status_update',$4,$5,'deterministic-success',
               ...
               'provider_disabled',false,'email-ai-pii/local-only',10,
               'local_only','deterministic-cost/1.0.0',1,'AI_PROVIDER_DISABLED',$6,now())`,
      [uuidv7(), organizationId, caseId, 'd'.repeat(64), 'e'.repeat(64), foreignUserId],
    )).rejects.toMatchObject({
      code: '23503',
      constraint: 'email_ai_suggestion_runs_actor_fk',
    })
    await expect(pool.query(
      `UPDATE email_ai_suggestion_runs SET safe_error_code='AI_BUDGET_EXCEEDED' WHERE id=$1`,
      [runId],
    )).rejects.toMatchObject({ code: '23001' })
    await expect(pool.query(
      `DELETE FROM ai_usage_ledger WHERE email_suggestion_run_id=$1`,
      [runId],
    )).rejects.toMatchObject({ code: '23001' })
    await expect(pool.query(
      `INSERT INTO ai_usage_ledger
         (id,organization_id,case_id,run_id,email_suggestion_run_id,usage_module,
          provider_id,model_id,request_hash,input_characters,output_characters,
          estimated_cost_minor,status,pricing_version,started_at)
       VALUES ($1,$2,$3,NULL,NULL,'email_draft','deterministic-success',
               'local-email-fixture-v1',$4,10,0,1,'completed',
               'deterministic-cost/1.0.0',now())`,
      [uuidv7(), organizationId, caseId, 'f'.repeat(64)],
    )).rejects.toMatchObject({
      code: '23514',
      constraint: 'ai_usage_ledger_valid',
    })
  })

  it('0026 tenant, sürüm, alıcı ve append-only e-posta taslak kısıtlarını zorlar', async () => {
    const organizationId = uuidv7()
    const foreignOrganizationId = uuidv7()
    const userId = uuidv7()
    const foreignUserId = uuidv7()
    ...
    const draftId = uuidv7()
    const versionId = uuidv7()
    const otherDraftId = uuidv7()
    const otherVersionId = uuidv7()
    const foreignPhotoId = uuidv7()
    await pool.query(
      `INSERT INTO organizations (id,code,name)
       VALUES ($1,'p41-db','P41 DB'),($2,'p41-db-foreign','P41 DB Foreign')`,
      [organizationId, foreignOrganizationId],
    )
    await pool.query(
      `INSERT INTO users (id,organization_id,email,password_hash,display_name)
       VALUES ($1,$3,'p41-db@test.local','x','P41 DB'),
              ($2,$4,'p41-db-foreign@test.local','x','P41 DB Foreign')`,
      [userId, foreignUserId, organizationId, foreignOrganizationId],
    )
    await pool.query(
      `INSERT INTO cases
         (id,organization_id,office_year,office_sequence,office_number,case_type,
          workflow_stage,plate,plate_normalized)
       VALUES ($1,$3,2026,4101,'2026/4101','traffic','reporting','34 DB 041','34DB041'),
              ($2,$4,2026,4102,'2026/4102','traffic','reporting','35 DB 041','35DB041')`,
      [caseId, foreignCaseId, organizationId, foreignOrganizationId],
    )
    await pool.query(
      `INSERT INTO photos
         (id,organization_id,case_id,original_file_name,display_name,mime_type,byte_size,
          content_hash,storage_root_key,relative_path,source_type,status,hash_verified,size_verified,verified_at)
       VALUES ($1,$2,$3,'foreign.jpg','Foreign','image/jpeg',1,$4,'foreign-root',
               'sentetik/foreign.jpg','manual','ready',true,true,now())`,
      [foreignPhotoId, foreignOrganizationId, foreignCaseId, 'f'.repeat(64)],
    )
    await pool.query(
      `INSERT INTO email_drafts
         (id,organization_id,case_id,draft_type,created_by_user_id)
       VALUES ($1,$3,$4,'case_status_update',$5),
              ($2,$3,$4,'missing_document_request',$5)`,
      [draftId, otherDraftId, organizationId, caseId, userId],
    )
    await pool.query(
      `INSERT INTO email_draft_versions
         (id,organization_id,case_id,draft_id,draft_version,subject,body,template_version,
          source_type,preview_hash,created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'Sentetik konu','Sentetik gövde',
               'email-draft-template/1.0.0','deterministic_template',$5,$6)`,
      [versionId, organizationId, caseId, draftId, 'a'.repeat(64), userId],
    )
    await pool.query(
      `INSERT INTO email_draft_versions
         (id,organization_id,case_id,draft_id,draft_version,subject,body,template_version,
          source_type,preview_hash,created_by_user_id)
       VALUES ($1,$2,$3,$4,1,'Diğer sentetik konu','Diğer sentetik gövde',
               'email-draft-template/1.0.0','deterministic_template',$5,$6)`,
      [otherVersionId, organizationId, caseId, otherDraftId, 'b'.repeat(64), userId],
    )
    await pool.query(
      `INSERT INTO email_draft_recipients
         (id,organization_id,case_id,draft_id,draft_version_id,recipient_kind,address,ordinal)
       VALUES ($1,$2,$3,$4,$5,'to','hasar@example.test',1)`,
      [uuidv7(), organizationId, caseId, draftId, versionId],
    )
    await pool.query('UPDATE email_drafts SET current_version_id=$2 WHERE id=$1', [draftId, versionId])
    await expect(pool.query(
      'UPDATE email_drafts SET current_version_id=$2 WHERE id=$1',
      [otherDraftId, versionId],
    )).rejects.toMatchObject({ code: '23503', constraint: 'email_drafts_current_version_fk' })
    await expect(pool.query(
      `INSERT INTO email_draft_recipients
         (id,organization_id,case_id,draft_id,draft_version_id,recipient_kind,address,ordinal)
       VALUES ($1,$2,$3,$4,$5,'cc','other@example.test',2)`,
      [uuidv7(), organizationId, caseId, otherDraftId, versionId],
    )).rejects.toMatchObject({ code: '23503', constraint: 'email_draft_recipients_version_fk' })
    await expect(pool.query(
      `INSERT INTO email_draft_recipients
         (id,organization_id,case_id,draft_id,draft_version_id,recipient_kind,address,ordinal)
       VALUES ($1,$2,$3,$4,$5,'cc','HASAR@example.test',1)`,
      [uuidv7(), organizationId, caseId, draftId, versionId],
    )).rejects.toMatchObject({ code: '23514', constraint: 'email_draft_recipients_valid' })
    await expect(pool.query(
      `INSERT INTO email_draft_attachments
         (id,organization_id,case_id,draft_id,draft_version_id,resource_type,photo_id,ordinal)
       VALUES ($1,$2,$3,$4,$5,'photo',$6,1)`,
      [uuidv7(), organizationId, caseId, draftId, versionId, foreignPhotoId],
    )).rejects.toMatchObject({ code: '23503', constraint: 'email_draft_attachments_photo_fk' })
    await expect(pool.query(
      'UPDATE email_draft_versions SET subject=$1 WHERE id=$2',
      ['Sessiz değişiklik', versionId],
    )).rejects.toMatchObject({ code: '23001' })
    await expect(pool.query(
      'UPDATE email_drafts SET version=3 WHERE id=$1',
      [draftId],
    )).rejects.toMatchObject({ code: '23514' })
  })
