...
const LABELS = [
  ...
  'Motor No', 'Şasi No', 'Eposta', 'Cep Telefonu', 'Kimlik Tipi', 'Kimlik No', 'Ad/Ünvan', 'Soyad', 'İş Telefonu',
...
export function parseEksist(text: string): EksistExtraction {
  ...
  matches.forEach((match, index) => {
    ...
    const value = clean.slice(match.index! + match[0].length, matches[index + 1]?.index ?? clean.length).trim().split('\u0000')[0]!.split(/©|Kayıt bulunamadı|\d{2}\.\d{2}\.\d{4} \d{2}:\d{2}/)[0]!.trim().replace(/\s+/g, ' ')
  ...
  const dateMatch = /^(\d{2})[/.](\d{2})[/.](\d{4})(?:\s|$)/.exec(date)
  const lossDate = dateMatch ? `${dateMatch[3]}-${dateMatch[2]}-${dateMatch[1]}` : /^\d{4}-\d{2}-\d{2}(?:\s|$)/.test(date) ? date.slice(0, 10) : ''
