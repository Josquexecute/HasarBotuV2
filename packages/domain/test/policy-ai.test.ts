...
describe('kanıtlı policy AI domain çekirdeği',()=>{
  ...
  it('PII minimizasyonu deterministik hash ve stabil placeholder üretirken poliçe şartını korur',()=>{const sources=[{sourceAnchorId:'a'.repeat(64),text:'Muafiyet %10.\nSigortalı: Ayşe Yılmaz\nE-posta: ayse@example.test\nTelefon: 0532 111 22 33\nPlaka: 34 ABC 123'}],first=minimizePolicyAiSources(sources),second=minimizePolicyAiSources(sources);expect(first).toEqual(second);expect(first.redactedCategories).toEqual(expect.arrayContaining(['email','name','phone','plate']));expect(first.redactedValueCount).toBeGreaterThanOrEqual(4);expect(first.sources[0]?.text).toContain('Muafiyet %10');expect(first.sources[0]?.text).not.toMatch(/Ayşe|example\.test|0532|34 ABC 123/);expect(first.outboundPayloadHash).toMatch(/^[a-f0-9]{64}$/)})
