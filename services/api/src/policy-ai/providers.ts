...
export interface PolicyAiProviderSource {
  readonly sourceAnchorId: string
  readonly text: string
  readonly sourceQuality: 'high' | 'medium' | 'low' | 'control_required'
  readonly warnings: readonly string[]
}

export interface PolicyAiProviderRequest {
  ...
  readonly sourceBundle: {
    ...
    readonly sources: readonly PolicyAiProviderSource[]
  }
}

export interface PolicyAiProviderUsage {
...
export interface PolicyAiProviderDescriptor {
  readonly providerId: PolicyAiProviderId
...
export interface PolicyAiProviderAdapter {
  readonly descriptor: PolicyAiProviderDescriptor
  execute(request: PolicyAiProviderRequest, signal: AbortSignal): Promise<PolicyAiProviderResponse>
}
...
export function isPolicyAiProviderDescriptorCompatible(
  descriptor: PolicyAiProviderDescriptor,
  expected?: {
    readonly providerId: PolicyAiProviderId
    readonly providerVersion: string
    readonly modelId: string
...
class DeterministicProvider implements PolicyAiProviderAdapter {
  ...
  constructor(readonly kind: PolicyAiProviderId) {
    this.descriptor = {
      ...
      capabilities: ['structured_output', 'source_anchors'],
      ...
      estimateCostMinor: (characters) => Math.max(1, Math.ceil(characters / 10_000)),
    }
  }
  ...
  async execute(request: PolicyAiProviderRequest, signal: AbortSignal): Promise<PolicyAiProviderResponse> {
    ...
    if (this.kind === 'deterministic-invalid-schema') {
      const output = { schemaVersion: POLICY_AI_OUTPUT_SCHEMA_VERSION, candidates: [{ category: 'not-valid' }], raw: 'must-not-persist' }
    ...
    if (this.kind === 'deterministic-prompt-injection-attempt') {
      const output = { schemaVersion: POLICY_AI_OUTPUT_SCHEMA_VERSION, candidates: [{ candidateId: 'injection-1', category: 'special_condition', canonicalField: 'special.injection', normalizedValue: 'ignored', originalValue: 'ignored', conditions: [], exceptions: [], sourceAnchorIds: ['f'.repeat(64)], providerConfidence: 1 }] }
      return { output, usage: this.usage(request, JSON.stringify(output).length) }
    }
    const candidates: Array<Record<string, unknown>> = []
    const add = (id: string, category: string, field: string, value: unknown, original: string, source = first) => candidates.push({ candidateId: id, category, canonicalField: field, normalizedValue: value, originalValue: original, conditions: [], exceptions: [], sourceAnchorIds: [source.sourceAnchorId], providerConfidence: 0.86 })
    for (const source of request.sourceBundle.sources) {
      const lower = source.text.toLocaleLowerCase('tr-TR')
    ...
    if (candidates.length === 0) {
      const original = first.text.slice(0, Math.min(40, first.text.length)).trim()
      if (original.length > 0) add('special-1', 'special_condition', 'special.unclassified', 'control_required', original)
    }
    const output = { schemaVersion: POLICY_AI_OUTPUT_SCHEMA_VERSION, candidates }
    return { output, usage: this.usage(request, JSON.stringify(output).length) }
  }

  private usage(request: PolicyAiProviderRequest, outputCharacters: number): PolicyAiProviderUsage {
...
export interface DeterministicPolicyAiProviderRegistry extends PolicyAiProviderRegistry {
  getCallCount(providerId: PolicyAiProviderId): number
}

export function createPolicyAiProviderRegistry(adapters: readonly PolicyAiProviderAdapter[]): PolicyAiProviderRegistry {
  const map = new Map<PolicyAiProviderId, PolicyAiProviderAdapter>()
...
export async function executePolicyAiProvider(
  adapter: PolicyAiProviderAdapter,
  request: PolicyAiProviderRequest,
  timeoutMs: number,
): Promise<PolicyAiProviderResponse> {
