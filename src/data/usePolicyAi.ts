...
import type { DataSourceKind, PolicyAiCandidateCategory, PolicyAiWorkspaceRecord } from './ports'

export type PolicyAiLoadStatus = 'idle' | 'loading' | 'ok' | 'empty' | 'unauthorized' | 'forbidden' | 'not_found' | 'conflict' | 'unavailable'
type BusyAction = 'plan' | 'start' | null
...
export function usePolicyAi(caseId: string, source: DataSourceKind) {
  ...
  const plan = useCallback(async () => {
    ...
    try {
      ...
      planAttempt.current = null
      startAttempt.current = null
      setWorkspace({ ...workspace, run: planned, candidates: [], conflicts: [] })
      setStatus('ok')
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      startAttempt.current = null
      setWorkspace({ ...workspace, run: started })
      setStatus('ok')
      setVersion((value) => value + 1)
    } catch (error) {
      fail(error)
  ...
  return { workspace, status, busy: busyAction !== null, busyAction, filter, setFilter, candidates, plan, start, retry }
}
