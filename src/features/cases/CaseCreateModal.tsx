...
export function CaseCreateModal({ currentUser, onClose, onUnauthorized, commandPort }: CaseCreateModalProps) {
        ...
        <form onSubmit={submit} noValidate>
          ...
          <div className="modal__body case-form-scroll">
            ...
            <div className="case-form-grid">
              ...
              <label className="form-field"><span>Sorumlu</span><select value={responsibleUserId} onChange={(event) => setResponsibleUserId(event.target.value)}><option value="">Atanmadı</option><option value={currentUser.id}>{currentUser.displayName} · oturum kullanıcısı</option></select><FieldError message={fieldErrors.responsibleUserId} /></label>
              <label className="form-field"><span>Sigorta şirketi kimliği</span><input value={insurerId} onChange={(event) => setInsurerId(event.target.value)} placeholder="Opsiyonel referans kimliği" autoComplete="off" aria-invalid={fieldErrors.insurerId !== undefined} /><small>Liste endpoint’i yok; yalnız bilinen gerçek kimlik girilebilir.</small><FieldError message={fieldErrors.insurerId} /></label>
              <label className="form-field"><span>Servis kimliği</span><input value={serviceId} onChange={(event) => setServiceId(event.target.value)} placeholder="Opsiyonel referans kimliği" autoComplete="off" aria-invalid={fieldErrors.serviceId !== undefined} /><small>Liste endpoint’i yok; yabancı tenant referansı reddedilir.</small><FieldError message={fieldErrors.serviceId} /></label>
              <label className="form-field form-field--unsupported"><span>Eksper</span><input value="Mevcut Case contract’ında ayrı alan yok" readOnly disabled /></label>
              <label className="form-field form-field--unsupported"><span>Hasar tarihi</span><input value="Mevcut Case contract’ında desteklenmiyor" readOnly disabled /></label>
              <label className="form-field form-field--unsupported"><span>İhbar tarihi</span><input value="Mevcut Case contract’ında desteklenmiyor" readOnly disabled /></label>
            </div>
          </div>
          <footer className="modal__footer">
            <button className="button button--secondary" type="button" onClick={onClose} disabled={submitting}>İptal</button>
            <button className="button button--primary" type="submit" disabled={submitting}>{submitting ? <><LoaderCircle className="spin" size={16} /> Kaydediliyor…</> : <><FilePlus2 size={16} /> Dosyayı Oluştur</>}</button>
          </footer>
        </form>
      </section>
    </div>
