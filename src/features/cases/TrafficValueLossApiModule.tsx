...
export function TrafficValueLossApiModule({
  ...
  const buildInput = async (): Promise<TrafficValueLossDraftInput> => {
    ...
    <div className="value-loss-grid">
      <div className="value-loss-main">
        <section className="info-panel value-loss-form-panel">
          <header><div><h2>Hesaplama Girdileri</h2><span>Para alanları TL; API minor-unit olarak alır</span></div><Calculator size={16} /></header>
      ...
      <aside className="value-loss-side">
        <section className="info-panel value-loss-result">
          <header><h2>Hesaplama Taslağı</h2>{shownVersion && <span className={`status-pill ${statusClass(shownVersion.status)}`}>{STATUS_LABELS[shownVersion.status] ?? shownVersion.status}</span>}</header>
          {shownVersion === null ? <p>Henüz sürümlü taslak yok. Girdileri ve kanıtları ekleyip hesaplayın.</p> : <>
            ...
            <details><summary>Kural kaynakları</summary>{shownVersion.evaluation.ruleSources.map((sourceItem) => <a key={sourceItem.code} href={sourceItem.url} target="_blank" rel="noreferrer"><strong>{sourceItem.title}</strong><span>{sourceItem.locator}</span></a>)}</details>
          </>}
        </section>
        ...
        <section className="info-panel value-loss-approval">
          <header><h2>Submit ve İnsan Onayı</h2><ShieldCheck size={16} /></header>
